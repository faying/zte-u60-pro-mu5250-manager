...
装 SSH 时会顺手**关掉固件自动升级**，原因见下面「注意事项」。
...
1. 电脑连上 U60 的 **Wi-Fi**（别只靠 USB 网络共享，开调试后 USB 那条网会断），输入路由器管理密码 → 脚本通过网页接口打开 USB 调试
...
然后说「帮我把 U60 装好」。它会先检查 adb、网络和固件，再问你要密码。
不想把密码告诉它的话，自己 `cp u60.env.example u60.env` 填好，跟它说「密码在 u60.env 里」就行。
插 USB 线、确认要不要重启这类事它会停下来问你。
...
- **别升级固件**。升级会覆盖 `rc.local`（SSH、后台、屏幕全部不再自启），而新固件又开不了 ADB，就回不去了。装 SSH 时已经关了自动升级；手机 App 或网页里提示升级也别点。
...
**装到一半失败**：脚本可以反复跑，已经装好的部分会跳过或覆盖。把终端输出整段发给给你装机包的人。
