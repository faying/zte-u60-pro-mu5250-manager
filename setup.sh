...
if [ "$(rcmd "test -f $STARTUP_SCRIPT && grep -qF '${SAFE_PASSWORD}' $STARTUP_SCRIPT 2>/dev/null && echo yes" | tr -d '\r')" = "yes" ]; then
...
start-stop-daemon -S -b -m -p /tmp/zte-agent.pid -x /bin/sh -- -c 'exec /data/zte-agent >/tmp/zte-agent.log 2>&1'
BOOT
...
if [ "$(rcmd "grep -qF '$RC_LINE' /etc/rc.local 2>/dev/null && echo yes" | tr -d '\r')" = "yes" ]; then
...
if [ "$SETUP_SSH" = "y" ] || [ "$SETUP_SSH" = "Y" ]; then
    ...
    # Probe well-known locations for an existing dropbear binary
    DROPBEAR_BIN=$(adb shell "for p in /usr/sbin/dropbear /data/dropbear; do [ -x \$p ] && echo \$p && break; done" 2>/dev/null | tr -d '\r\n ')
    if [ -n "$DROPBEAR_BIN" ]; then
        ok "Dropbear already installed at $DROPBEAR_BIN."
    else
        info "Downloading dropbear for aarch64..."
        DROPBEAR_DIR="https://downloads.openwrt.org/releases/23.05.4/targets/armsr/armv8/packages"
        DROPBEAR_FILE=$(curl -sfL "$DROPBEAR_DIR/" \
            | grep -oE 'dropbear_[^"]*_aarch64_generic\.ipk' \
            | grep -v dropbearconvert | head -1)
        [ -z "$DROPBEAR_FILE" ] && fail "Could not find dropbear .ipk in $DROPBEAR_DIR/"
        DROPBEAR_URL="$DROPBEAR_DIR/$DROPBEAR_FILE"
        TMPDIR=$(mktemp -d)
        curl -sfL "$DROPBEAR_URL" -o "$TMPDIR/dropbear.ipk" || fail "Failed to download dropbear from $DROPBEAR_URL"
        # .ipk is a gzipped tar of {debian-binary, data.tar.gz, control.tar.gz}
        info "Extracting dropbear binary from .ipk..."
        ( cd "$TMPDIR" && tar xzf dropbear.ipk && tar xzf data.tar.gz ) \
            || fail "Failed to extract dropbear .ipk."
        [ -x "$TMPDIR/usr/sbin/dropbear" ] || fail "dropbear binary not found inside package."
        adb push "$TMPDIR/usr/sbin/dropbear" /data/dropbear >/dev/null \
            || fail "Failed to push dropbear to device."
        adb shell "chmod +x /data/dropbear"
        rm -rf "$TMPDIR"
        DROPBEAR_BIN=/data/dropbear
        ok "Dropbear installed at $DROPBEAR_BIN."
    fi
    ...
    PUBKEY=$(cat "$SSH_PUB")
    if [ "$(adb shell "test -f /etc/dropbear/authorized_keys && grep -qF '$PUBKEY' /etc/dropbear/authorized_keys && echo yes" 2>/dev/null | tr -d '\r')" = "yes" ]; then
    ...
    if [ "$(adb shell "test -x $DROPBEAR_STARTUP && grep -qF '$DROPBEAR_BIN' $DROPBEAR_STARTUP && echo yes" 2>/dev/null | tr -d '\r')" = "yes" ]; then
        ok "Dropbear startup script already exists."
    else
        info "Creating dropbear startup script..."
        cat > /tmp/start_dropbear.sh <<DBBOOT
#!/bin/sh
start-stop-daemon -S -b -m -p /tmp/dropbear.pid -x $DROPBEAR_BIN -- -p 2222 -R -F
DBBOOT
        adb push /tmp/start_dropbear.sh "$DROPBEAR_STARTUP" >/dev/null
        adb shell "chmod +x $DROPBEAR_STARTUP"
        rm /tmp/start_dropbear.sh
        ok "Dropbear startup script created."
    fi

    # Add to rc.local if not already there
    DB_RC_LINE="sh $DROPBEAR_STARTUP"
    if [ "$(adb shell "grep -qF '$DB_RC_LINE' /etc/rc.local 2>/dev/null && echo yes" | tr -d '\r')" = "yes" ]; then
    ...
    # Start dropbear now
    if [ -n "$(adb shell "pidof dropbear" 2>/dev/null | tr -d '\r\n ')" ]; then
        ...
        if [ -n "$(adb shell "pidof dropbear" 2>/dev/null | tr -d '\r\n ')" ]; then
            ...
            warn "Dropbear failed to start. Check 'adb shell logread | grep dropbear'."
        fi
    fi
...
# ── Step 10: Build & deploy admin UI (optional) ──────────────────────
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
WEB_DIR="$SCRIPT_DIR/web"
UI_REMOTE=/data/admin
DEPLOY_UI=true
if ! command -v npm >/dev/null 2>&1; then
    warn "npm not found — skipping admin UI build. Install Node.js (brew install node) to enable the web UI."
    DEPLOY_UI=false
elif [ ! -d "$WEB_DIR" ]; then
    warn "web/ directory not found — skipping admin UI build."
    DEPLOY_UI=false
fi
...
if [ "$DEPLOY_UI" = true ]; then
    ...
    ) || { warn "UI build failed — admin UI will not be deployed."; DEPLOY_UI=false; }
fi

if [ "$DEPLOY_UI" = true ] && [ -d "$WEB_DIR/out" ]; then
    info "Deploying admin UI to $UI_REMOTE..."
    UI_TAR=$(mktemp -t u60ui.XXXXXX.tar.gz)
    trap 'rm -f "$UI_TAR"' EXIT
    tar czf "$UI_TAR" -C "$WEB_DIR/out" . || fail "Failed to package web/out"

    if [ "$USE_SSH" = true ]; then
        $SSH_CMD "rm -rf $UI_REMOTE && mkdir -p $UI_REMOTE" \
            && cat "$UI_TAR" | $SSH_CMD "cd $UI_REMOTE && tar xzf -" \
            || { warn "SSH push of UI failed."; DEPLOY_UI=false; }
    else
        adb push "$UI_TAR" /data/admin.tar.gz >/dev/null \
            && adb shell "rm -rf $UI_REMOTE && mkdir -p $UI_REMOTE && cd $UI_REMOTE && tar xzf /data/admin.tar.gz && rm /data/admin.tar.gz" \
            || { warn "ADB push of UI failed."; DEPLOY_UI=false; }
    fi

    if [ "$DEPLOY_UI" = true ]; then
        ok "Admin UI deployed to $UI_REMOTE."
        info "Restarting agent to pick up UI dir env (idempotent)..."
        rcmd "killall zte-agent 2>/dev/null; sleep 1; sh $STARTUP_SCRIPT"
    fi
fi
